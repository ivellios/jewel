...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        game_copies: Iterable[GameOnPlatform] = GameOnPlatform.objects.filter(platform=steam_platform, identifier__isnull=True)
        matches = []
        multiple_matches = []
        for game_copy in tqdm(game_copies):
            ...
            if score == 100:
                ...
                game_copy.save()
            if score < 100:
                matches.append((game.title, game.id, steam_game.name, steam_game.appid, score))
        ...
        for game in matches:
            pprint(f"Match: {game[0]}({game[1]}) - {game[2]} (https://store.steampowered.com/api/appdetails?appids={game[3]}) | score: {game[3]}")
